...
var d = {
  ...
  function updateElements() {
    ...
    Object.keys(elemPresence).forEach(key => {elemPresence[key] = 0})
    Object.keys(formulae).forEach(key => {
      let formula = formulae[key]
      elemPresence[formula.elements[0]]++
      if (formula.elements[0] != formula.elements[1]) {elemPresence[formula.elements[1]]++}
    })
        ...
        if (basicElements.includes(key)) {basicElementsBox.appendChild(elemDiv)}
          ...
          if (elemPresence[key] == 0) {
            if (d.gid('search_input').value != '') {
              if (key.toUpperCase().indexOf(d.gid('search_input').value.toUpperCase()) > -1) {finalElementsBox.appendChild(elemDiv)}
            } else {finalElementsBox.appendChild(elemDiv)}
          } else {
            if (d.gid('search_input').value != '') {
  ...
  function changeCSS(variable,value) {document.documentElement.style.setProperty(("--").concat(variable),value)}
  var theme = 0
